...
def get_pages_links():
    ...
    last_page = int(
        bs.find_all(
            ...
            {
                "href": re.compile(
                    "https:\/\/www.otomoto\.pl/osobowe\/opel\/astra\/\?search%5Border%5D=created_at%3Adesc&page=[0-9]{1,3}"
                )
            },
        )[-2].span.text
    )
    ...
    start_urls = [
        url + str(i) for i in range(1, last_page + 1)
...
def get_offer_links(urls, only_100=True):
    ...
    urls - list of links to pages
    only_100 - if True only links to first 100 offers will be scraped
    """
    offer_links = []  # empty result list
    print("Creating list of links to all offers: IN PROGRESS...")
    for (
        url
    ) in urls:  # iterating over links to pages scraped by get_pages_links() function
        html = request.urlopen(url)
        bs = BS(html.read(), "html.parser")
        chunks = bs.find_all(
            "a", {"class": "offer-title__link"}
        )  # creating list of html chunks where links are stored
        for x in chunks:
            offer_links.append(
                x["href"]
